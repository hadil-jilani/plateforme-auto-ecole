...
@Inject('Demande') private Sender : ClientProxy
){}
async GetRequests(){
  return  await this.EcoleModel.find({status : status.EN_ATTENTE}).select('-password')   
 }

async Accept_demande(id: string){
  const Id = new ObjectId(id);
  const user = await this.EcoleModel.findById(Id).select('-password');
if (!user){
      throw new RpcException(new BadRequestException(' No demande found with this user id  ! '));
  }
